...
- **メニューバー**: 本日のコスト（例 `$12.34`）をライブ表示。表示は切替可能
  （price / tokens / 2段組み / 週次残量 — 週次残量は残額と**予算に対する割合**を
  併記: `$116 · 58%`）
- **ポップオーバー**（クリック）: 本日のコスト、input/output/cache トークン、
  直近30日合計、週次バジェットバー（有効時）
- **未価格の使用量は隠さず表示**: 本来コストがかかるはずの $0 のターン（同梱 CLI の
  単価表より新しいモデル、または単価更新をまだ履歴に適用していない状態）がストアに
  あると、メニューバーの数値に `⚠︎` が付き、ポップオーバーが件数とモデル名を示します。
  **Reprice** ボタンで現在の単価を保存済み履歴に適用できます。それでも残る場合は
  アプリの更新が必要です
- **バージョン**: 起動中ビルドの版数をポップオーバー下部に表示（選択・コピー可）。
  メニューバーアプリには `--version` に相当する導線が無いため
...
- **週次バジェットモニタ**（任意）: 週の使用量を **コスト($) / トークン(in+out)** 基準で
  ...
  ポップオーバーの "Settings…"。使用分・残量とも**金額と%を併記**します
  （`68% used · $1,200.00 left (32%)`）。通知を ON にした時点で macOS に許可を
  求めます。システム設定でこのアプリの通知がオフになっている場合は、トグルの下に
  その旨を表示し、通知ペインを開く **Open Settings** ボタンを出します。
- **超過しそうか**: ポップオーバーは、ここまでのペースから週の着地点も予測します
  （`On pace for $5,200.00 (137%) — budget gone Sat 14:26`）。窓の経過分に対する
  消費を平均レートとみなし、リセットまで線形に外挿したものです。この行の色は
  **予測値の**深刻度で決まるため、現時点の使用量はまだしきい値内でも、
  このままでは超過する週はバーより先に警告色になります。窓の最初の約8時間は
  1セッションが平均を支配してしまうため、無茶な数字を出さず
  `Too early this week to project a pace` と表示します。
