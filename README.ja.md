...
- **週次バジェットモニタ**（任意）: Claude の実リミットは読めないので**自分でバジェット
  設定**。**コスト($) / トークン(in+out)** 基準、**リセット曜日/時刻**（ローカル）、
  **2段階**（警告/危険）しきい値。近づくとメニューバーの数値が橙/赤に変色し、ポップオーバーに
  色付きの使用/上限バーが出て、（任意で）**通知**。設定は ⌘, / ポップオーバーの "Settings…"。
